// Renaming file to main.jsx as requested by user's focus, and making it the monolithic component.
import React, { useState, useEffect } from 'react';
// Removing ReactDOM import as the environment handles rendering the default export component.
import { 
  Calendar, 
  BookOpen, 
  Sparkles, 
...
// --- STYLES (Injected) ---
// We inject these styles dynamically to satisfy the single-file requirement
const customStyles = `
...
// The primary component must be named App and exported as default for the environment to run it.
export default function App() {
  ...
  useEffect(() => {
    ...
    // 2. Init Firebase
    try {
      // Ensure __firebase_config is a string and needs parsing
      const firebaseConfig = JSON.parse(typeof __firebase_config === 'string' ? __firebase_config : '{}');
      ...
      setError("Configuration error. Please reload.");
    }
  }, []);

  // 4. Setup Speech Recognition (Separate Effect)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Data is stored in the private collection path: 
    // /artifacts/{appId}/users/{userId}/journal_entries
    const q = query(
  ...
  const toggleRecording = () => {
    ...
    if (isRecording) {
      ...
      // Clear previous text if starting new recording, otherwise append
      // setText(''); 
      recognition.start();
      setIsRecording(true);
    }
            ...
            {insight ? (
              <div className="bg-white rounded-2xl p-6 shadow-xl border border-slate-200">
                ...
                <div className="text-slate-700 leading-relaxed whitespace-pre-wrap">
                  {/* The output is treated as pre-formatted text (with bolding/bullets from Gemini) */}
                  {insight.split('\n').map((line, i) => (
